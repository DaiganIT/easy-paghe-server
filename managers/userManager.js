...
import { ValidationException } from '../queries/validationException';
import { InvalidOperationException } from '../queries/invalidOperationException';
...
export default class UserManager {
  ...
  constructor(db) {
		if (!db instanceof Connection)
			throw 'Database is not a valid object';
		this.db = db;
	}
	...
	addAsync(userModel) {
		...
		return hashPromise.then((hash) => {
			...
			return this.db
				...
				.save(user)
				.then((user) => {
					if (!user.active) Mailer.sendActivationMessage(user.username, user.activationCode);
					return user;
				})
	...
	async activateUserAsync(code) {
		let user = await this.db.getRepository(User).findOne({ activationCode: code });
		if (user) {
...
function validateUser(user) {
	const errors = [];
	if (validator.isEmpty(user.username)) errors.push({ key: 'username', message: 'Inserisci un valore' });
	if (!validator.isEmail(user.username)) errors.push({ key: 'username', message: 'Deve essere una email valida' });
	if (validator.isEmpty(user.password)) errors.push({ key: 'password', message: 'Inserisci una password' });
